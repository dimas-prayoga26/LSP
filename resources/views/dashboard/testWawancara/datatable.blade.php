<script>
    function getData() {
        const table = $('#table-testWawancara');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const deleteRoute = '{{ route("ujianWawancara.destroy", [":uuid"]) }}';
                const editRoute = '{{ route("ujianWawancara.edit", [":uuid"]) }}';
                    ...
                    "columns": [
                        {
                            ...
                            render: function(data) {
                                return data ? `<span class="text-wrap">${data}</span>` : 'N/A';
                            }
                        },
                        {
                            data: 'kriteria_unjuk_kerja',
                            render: function(data) {
                                return data ? `<span class="text-wrap">${data.nama_kriteria_kerja}</span>` : 'N/A';
                            }
                        },
                        {
                            data: 'unit_kompetensi',
                            render: function(data) {
                                return data ? `<span class="text-wrap">${data.judul_unit}</span>` : 'N/A';
                            }
                        },
                        {
                            data: 'uuid',
